...
CREATE TABLE CODE_GROUP (
    groupId NUMBER NOT NULL,          -- 그룹 ID
    groupCode VARCHAR2(50),           -- 그룹 코드
    groupName VARCHAR2(100),          -- 그룹명
    CONSTRAINT PK_CODE_GROUP PRIMARY KEY (groupId)
);


/* 공통코드_코드 */
CREATE TABLE CODE_VALUE (
    valueId NUMBER NOT NULL,          -- 코드 ID → valueId로 변경
    valueCode VARCHAR2(50),           -- 코드 → valueCode로 변경
    valueName VARCHAR2(200),          -- 코드명 → valueName으로 변경
    parentValueId NUMBER,             -- 상위 코드 ID (컬럼명 일관성 유지)
    groupId NUMBER,                   -- 그룹 ID
    CONSTRAINT PK_CODE_VALUE PRIMARY KEY (valueId),

    -- 그룹 테이블과의 관계
    CONSTRAINT FK_CODE_VALUE_GROUP FOREIGN KEY (groupId)
        REFERENCES CODE_GROUP (groupId)
        ON DELETE CASCADE,

    -- 자기 자신(parentValueId) 참조 (계층 구조)
    CONSTRAINT FK_CODE_VALUE_PARENT FOREIGN KEY (parentValueId)
        REFERENCES CODE_VALUE (valueId)
        ON DELETE SET NULL
);
