...
EXEC up_user_exception(  6000, 7000 ); 

--동적쿼리 -> 쿼리가 사용자에 의해서 완성
--동적쿼리를 사용하는 3가지경우
1) 컴파일시에 SQL문장이 확정되지않은경우
WHERE 조건절 XXX
--> 사용자의 선택에 의해서 WHERE 조건절이 결정...
...
--예) 동적쿼리를 사용해서 DEPT 테이블에 부서를 추가 예제
-- 예4) 동적쿼리를 사용해서 dept 테이블에 부서를 추가 예제
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('부서 추가 성공!!');  
-- COMMIT; -- 안한이유 : ROLLBACK; 할 예정
-- EXCEPTION
END;
...
IS
   vsql VARCHAR2(2000);
   vrow emp%ROWTYPE;
   vcur SYS_REFCURSOR;   -- 오라클 9i 이전  REF CURSOR
BEGIN
  vsql := 'SELECT * ';
  vsql := vsql || ' FROM emp ';
  vsql := vsql || ' WHERE ';
  ...
  ELSIF psearchCondition = 2 THEN
    vsql := vsql || ' REGEXP_LIKE( ename , :psearchWord, ''i'' ) ';
  ELSIF psearchCondition = 3 THEN
    vsql := vsql || ' REGEXP_LIKE( job , :psearchWord, ''i'' ) ';
  END IF;
...
------------------
-- 견학신청(TOUR_APPLICATION) 테이블의 PK를 위한 시퀀스
CREATE SEQUENCE SEQ_TOUR_APPLICATION
 START WITH 5003 -- 마지막 더미데이터 5002 다음 번호
 INCREMENT BY 1
 NOCACHE;

-- 견학통계(TOUR_STATISTIC) 테이블의 PK를 위한 시퀀스
CREATE SEQUENCE SEQ_TOUR_STATISTIC
 START WITH 50003 -- 마지막 더미데이터 50002 다음 번호
 INCREMENT BY 1
 NOCACHE;

-- 견학 신청시 통계 자동 추가 트리거
CREATE OR REPLACE TRIGGER trg_add_tour_statistic
    AFTER INSERT ON TOUR_APPLICATION -- '견학신청'이 INSERT 된 후에
    FOR EACH ROW
BEGIN
    INSERT INTO TOUR_STATISTIC (tourStatId, tourId, userId)
    VALUES (
        SEQ_TOUR_STATISTIC.NEXTVAL,
        :NEW.tourId,
        :NEW.userId
    );
END;
...
-- 견학 취소시 통계 자동 삭제 트리거
CREATE OR REPLACE TRIGGER trg_delete_tour_statistic
    BEFORE DELETE ON TOUR_APPLICATION
    FOR EACH ROW
BEGIN
    DELETE FROM TOUR_STATISTIC
...
VALUES (SEQ_TOUR_APPLICATION.NEXTVAL, 10, DATE '2025-11-10', '예약완료', 1, 3);
COMMIT;
SELECT * FROM TOUR_STATISTIC;

-- 견학신청 취소
DELETE FROM TOUR_APPLICATION WHERE userId = 3;
COMMIT;
SELECT * FROM TOUR_STATISTIC;
...
---------------------------------------------------
DECLARE
    -- 1. 커서(Cursor)에 한글 별칭을 정의합니다.
    CURSOR c_facility_hours IS
        SELECT 
            a.amenityName AS "자료실",  -- 별칭 1
            o.holidayInfo AS "휴일",    -- 별칭 2
            (TO_CHAR(o.openTime, 'HH24:MI') || ' ~ ' || TO_CHAR(o.closeTime, 'HH24:MI')) AS "이용시간" -- 별칭 3
        FROM 
            AMENITY a
        JOIN 
            OPERATING_HOUR o ON a.amenityId = o.amenityId
        WHERE
            a.amenityId IN (1, 2, 3, 4) -- 1, 2, 3, 4번 ID
        ORDER BY 
            a.amenityId;

BEGIN
    -- 2. 헤더(별칭과 동일한 텍스트)를 직접 출력합니다.
    DBMS_OUTPUT.PUT_LINE(RPAD('자료실', 15) || ' | ' || RPAD('이용시간', 20) || ' | ' || '휴일');
    DBMS_OUTPUT.PUT_LINE('-----------------------------------------------------------------------------');

    -- 3. 커서 루프를 돌면서 한 줄씩 출력
    FOR v_row IN c_facility_hours LOOP
        
        -- 4. v_row에서 컬럼명 대신 '별칭'을 사용해서 데이터를 가져옵니다.
        --    (별칭에 한글이나 공백이 있으면 "큰따옴표"로 묶어야 함)
        DBMS_OUTPUT.PUT_LINE(
            RPAD(v_row."자료실", 15) || ' | ' || 
            RPAD(v_row."이용시간", 20) || ' | ' || 
            v_row."휴일"
